...
bool recompile_single_function(const N64Recomp::Context& context, const N64Recomp::Function& func, const std::filesystem::path& output_path, std::span<std::vector<uint32_t>> static_funcs_out) {
    ...
    if (!N64Recomp::recompile_function(context, func, output_file, static_funcs_out, true)) {
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < context.functions.size(); i++) {
        ...
        if (!func.ignored && func.words.size() != 0) {
            ...
            bool result;
            if (config.single_file_output) {
                result = N64Recomp::recompile_function(context, func, single_output_file, static_funcs_by_section, false);
            }
            else {
